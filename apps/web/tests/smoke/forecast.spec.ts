...
test("/itamambuca renders 13 hour rows with parseable scores", async ({ page }) => {
  ...
  // Every row starts with `NNh` and the daylight slice is 06h..18h = 13 rows.
  const rowTimes = await page
    .locator(".layout-desktop")
    .getByText(/^\d{2}h$/)
    .allTextContents();
  // Drop top-bar / hero "hoje" type cells — the table-row times should each
  // appear at least once; assert we see the full 06h..18h sequence.
  const distinctHours = Array.from(new Set(rowTimes));
  for (const h of ["06h", "12h", "18h"]) {
    expect(distinctHours).toContain(h);
  }
  expect(distinctHours.length).toBeGreaterThanOrEqual(13);
});
