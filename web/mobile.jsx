...
(function () {
  ...
  const { C, AppBar, SummaryCard, HourList, FilterChips, SuggestionPill } = window.MS;
  const D = window.partiuData;

  const SNAPS = { peek: 18, half: 52, full: 92 };
  ...
  function Sheet({state, setState}) {
    const [livePct, setLivePct] = useState(null); // overrides height while dragging
    ...
    return (
      <div style={{
        ...
        transition: livePct == null ? 'height .3s ease, background .25s ease, color .25s ease' : 'none',
        overflow:'hidden',
        touchAction:'none',
        zIndex:2,
      }}>
        <div
          ...
          onClick={cycle}
          style={{padding:'10px 0 6px', display:'flex', justifyContent:'center', cursor:'pointer'}}
        ...
        {state === 'peek' && livePct == null && <PeekContents />}
  ...
  function PeekContents() {
    const sugg = D.suggestions.slice(0, 3);
    return (
      <div style={{padding:'4px 14px 16px'}}>
        <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:8}}>
          <span style={{
            width:26, height:26, borderRadius:'50%', background:C.coral, color:'#fff',
            display:'inline-flex', alignItems:'center', justifyContent:'center', fontSize:14,
          }}>✦</span>
          <span style={{fontSize:13, color:C.ink, fontWeight:600}}>Pergunta pro copiloto</span>
          <span style={{marginLeft:'auto', fontSize:11, color:C.inkSoft}}>↑ arraste</span>
        </div>
        <div style={{display:'flex', gap:6, overflowX:'auto', paddingBottom:4}}>
          {sugg.map((s,i)=>(<SuggestionPill key={i}>{s}</SuggestionPill>))}
        </div>
      </div>
    );
  }
  ...
  function HalfContents() {
    return (
      <>
        <div style={{padding:'4px 14px 6px', fontSize:11, color:C.inkSoft, fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase'}}>
          Sugestões
        </div>
        ...
        <div style={{padding:'8px 14px', flex:1, overflow:'auto'}}>
          <BotBubble>
            <b style={{color:C.deep}}>Bom dia! 🏄‍♀️</b><br/>
            Itamambuca tá rendendo <b style={{color:C.green}}>8.9</b> agora. Janela boa <b>até 10h</b>, depois o vento vira oeste.
          </BotBubble>
        </div>
        <Composer />
      </>
  ...
  function FullContents({onClose}) {
    return (
      <>
        <div style={{padding:'2px 16px 10px', display:'flex', alignItems:'center', gap:8}}>
          <span style={{
            width:8, height:8, borderRadius:'50%', background:'#7adcd2', boxShadow:'0 0 8px #7adcd2'
          }}/>
          ...
          <button
            ...
            aria-label="fechar"
            style={{
              marginLeft:'auto', fontSize:14, color:'rgba(255,255,255,0.6)',
              background:'transparent', border:'none', cursor:'pointer', padding:'4px 8px',
            }}>×</button>
        </div>
        <div style={{flex:1, padding:'4px 14px', overflow:'auto', display:'flex', flexDirection:'column', gap:10}}>
          <BotBubble dark>
            Bom dia! Itamambuca <b style={{color:'#7adcd2'}}>8.9/10</b> agora — pico até 10h.
          </BotBubble>
          <UserBubble dark>e pro meu fish 5'8"?</UserBubble>
          <BotBubble dark>
            Pra fish 5'8" o sweet spot é <b style={{color:'#7adcd2'}}>1.0–1.6m · 9–12s</b>. Hoje tá 1.7m·13s — um <b>nada</b> grande, mas dentro. <b>Score 8.4</b> pro fish (vs 8.9 pro short).
          </BotBubble>
  ...
  function BotBubble({children, dark, typing}) {
    return (
      <div style={{display:'flex', gap:8, alignItems:'flex-start'}}>
        ...
        <div style={{
          maxWidth:'82%', padding:'10px 12px', borderRadius:14, borderTopLeftRadius:4,
  ...
  function UserBubble({children, dark}) {
    return (
      <div style={{display:'flex', justifyContent:'flex-end'}}>
        <div style={{
          maxWidth:'80%', padding:'9px 12px', borderRadius:14, borderTopRightRadius:4,
          background: dark ? '#7adcd2' : C.deep, color: dark ? '#0a3a44' : '#fff',
  ...
  function Composer() {
    ...
    return (
      <form
        onSubmit={(e)=>{e.preventDefault(); setV('');}}
        style={{padding:'8px 14px calc(14px + env(safe-area-inset-bottom))'}}>
        <div style={{
          display:'flex', alignItems:'center', gap:8,
          background:'#fff', borderRadius:999, padding:'4px 4px 4px 14px',
          ...
          <input
            ...
            style={{
              flex:1, fontSize:13, color:C.ink, padding:'8px 0',
              background:'transparent', border:'none', outline:'none',
            }}
          />
          <button type="submit" aria-label="enviar" style={{
            width:30, height:30, borderRadius:'50%', background:C.coral, color:'#fff',
  ...
  function ComposerDark() {
    ...
    return (
      <form
        ...
        <div style={{
          display:'flex', alignItems:'center', gap:8,
          background:'rgba(255,255,255,0.08)', borderRadius:999, padding:'4px 4px 4px 14px',
        }}>
          <input
            value={v}
            onChange={(e)=>setV(e.target.value)}
            ...
            style={{
              flex:1, fontSize:13, color:'#fff', padding:'8px 0',
              background:'transparent', border:'none', outline:'none',
            }}
          />
          <button type="submit" aria-label="enviar" style={{
            width:30, height:30, borderRadius:'50%', background:'#7adcd2', color:'#0a3a44',
      ...
      <div style={{
        ...
        {dim && (
          <div
            ...
            style={{
              position:'absolute', inset:0, background:'rgba(10,42,50,0.35)',
              transition:'opacity .25s ease', zIndex:1,
            }}
          />
        )}
        <Sheet state={state} setState={setState} />
      </div>
    );
  }

  window.PartiuMobile = MobileApp;
})();
