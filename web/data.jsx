...
// Replace this module with a fetch() against a JSON endpoint when wiring real data.

window.partiuData = (function () {
  // Hourly forecast — 06h..18h, 13 rows
  const hours = [
    // h  score swH swT swDir wKmh wDir gust tideH tideState   flag
  ...
  const welcome = `Oi! Sou o copiloto do partiu.surf. Posso te ajudar a decidir se vale a pena ir surfar — analiso swell, vento, maré e cruzo com o seu nível e equipamento. Procura um pico ali na direita ou me pergunta direto.`;
  ...
  const scoreColor = (s) => s >= 7 ? 'green' : s >= 4 ? 'amber' : 'red';
  const scoreEmoji = (s) => s >= 7 ? '🟢' : s >= 4 ? '🟡' : '🔴';

  const COMPASS = ['N','NNE','NE','ENE','L','ESE','SE','SSE','S','SSO','SO','OSO','O','ONO','NO','NNO'];
  const dirLabel = (deg) => COMPASS[Math.round(((deg % 360) / 22.5)) % 16];

  return { hours, spots, suggestions, welcome, historic, spot, scoreColor, scoreEmoji, dirLabel };
})();
