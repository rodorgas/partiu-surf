...
**Monthly climatology** powers the "Comparado à média" card on the web: `surfcheck/climatology.py` averages swH, swT, and the composite score for the same calendar month across the past 3 years of Open-Meteo archive data. The web app exposes this through a separate `/api/climatology` function so it can be cached permanently per `(spot, YYYY-MM, gear)` — orthogonal to the 12h daily-forecast cache.
